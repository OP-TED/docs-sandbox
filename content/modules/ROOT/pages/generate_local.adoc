...
.. You are using the latest ui - you can also clone these locally or use the online ones as per the playbook settings on the acceptance site, docs-staging. See what these are online.
.. You have installed and have running the Live Server extension in VSC: then browse to \docs-sandbox\build\site locally (from the folder where docs-sandbox was cloned to locally) to view the site "http://127.0.0.1:5500/build/site/home/index.html"

From now on you can use docs-sandbox locally to generate the whole docs website, i.e. all repositories as if you were actually on the web. This will give you a better view that generating each docs repo in isolation while you are working on it.
